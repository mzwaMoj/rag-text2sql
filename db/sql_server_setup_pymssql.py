"""
SQL Server Customer Data Setup using pymssql
This script creates a customer information table in SQL Server and populates it with synthetic data.
...
def connect_to_sql_server():
    """Connect to SQL Server using pymssql with Windows authentication"""
    server = "localhost"
    database = "master"  # You might want to change this to a specific database
    
    try:
        # Connect using pymssql with Windows authentication
        # When using Windows authentication, don't provide username/password
        ...
        # Try with explicit server name formats
        try:
            print("Trying with server name as localhost...")
            conn = pymssql.connect(server="localhost", database=database)
            cursor = conn.cursor()
            print(f"Successfully connected to SQL Server database: {database}")
            return conn, cursor
            ...
            try:
                ...
                print(f"Trying with computer name: {computer_name}")
                conn = pymssql.connect(server=computer_name, database=database)
                cursor = conn.cursor()
                print(f"Successfully connected to SQL Server database: {database}")
                return conn, cursor
            except Exception as e3:
                print(f"Error with computer name: {e3}")
...
def insert_data_into_sql_server(conn, cursor, df, table_name="customer_information"):
    """Insert data from DataFrame into SQL Server table using pymssql"""
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                # Insert query with pymssql parameter style (%s)
                insert_query = f"""
...
def verify_sql_server_data(conn, cursor, table_name="customer_information"):
    ...
    try:
        ...
        row = cursor.fetchone()
        if row:
            print(f"Sample record - ID: {row[0]}, Name: {row[1]}, Email: {row[2]}")
        return count
    except Exception as e:
